...
\begin{figure}[htbp]
\centering
\input{figures/tikz/tikz_data_pipeline}
...
\input{figures/tables/tab_data_pipeline_scale}
\end{table}
...
\input{figures/tables/tab_chart_type_distribution}
\end{table}

\textbf{Step 4: Feature Extraction.}
The Stage~3 extraction pipeline (see \Cref{sec:system_s3}) processes all 32,364 charts.
For the initial training dataset (v3), the batch extraction was executed with the geometry-based
Stage~3 implementation, achieving 100\% success rate with 0\% data corruption. The VLM-based
Stage~3 redesign produces structured table representations (headers, rows, records) for
downstream reasoning; reprocessing the full dataset with VLM backends is planned for
future training data improvement.
...
\label{tab:splits}
\input{figures/tables/tab_dataset_splits}
\end{table}
...
\input{figures/tables/tab_slm_training_config}
\end{table}
...
\subsubsection{Hybrid Neural-VLM Design}

The core design of Geo-SLM combines three complementary paradigms:

\begin{itemize}
    \item \textbf{Deep Learning} for perception: detecting chart regions (YOLOv8) and
          classifying chart types (EfficientNet-B0, 97.54\% accuracy).
    \item \textbf{VLM Extraction} for chart derendering: converting chart images
          directly to structured data tables using Vision-Language Models (DePlot,
          MatCha). A formal benchmark evaluation on 50 real-world academic charts
          confirmed that the initial geometry-based approach (OCR, skeletonization,
          RANSAC axis calibration) is unsuitable for production use -- axis detection
          succeeded on 0 of 40 non-pie charts -- motivating the switch to end-to-end
          neural derendering.
    \item \textbf{Language Model Reasoning} for understanding: validating extracted
          data, correcting extraction errors, and generating natural language descriptions
          using fine-tuned Small Language Models (Qwen-2.5-1.5B / Llama-3.2-1B).
\end{itemize}
...
\begin{figure}[htbp]
\centering
\input{figures/tikz/tikz_approach_comparison}
...
\begin{figure}[htbp]
\centering
\input{figures/tikz/tikz_pipeline_architecture}
...
\label{tab:stages}
\input{figures/tables/tab_pipeline_stages}
\end{table}
...
\midrule
S3 & EfficientNet-B0 Classifier & No change (visual features, language-agnostic) & No \\
S3 & VLM Extractor (DePlot/MatCha) & No code change; model output quality may decrease for non-Latin labels & No (inherent model limitation) \\
S3 & VLM Extractor (SVLM/Qwen2-VL) & Configure system prompt for Vietnamese output & Config only \\
...
\subsubsection{Vietnamese Chart Labels and VLM Output}

Vietnamese uses Latin script with an extensive diacritical system (6~tone marks $\times$
12~vowel variants). In the VLM-based Stage~3, axis label text is extracted as part of
the chart-to-table derendering output rather than through a standalone OCR engine.
This changes the Vietnamese support profile:

\begin{itemize}
    \item \textbf{DePlot / MatCha}: These models were pre-trained on English chart data.
          Vietnamese text in axis labels may be transcribed with diacritic errors.
          Stage~4 SLM correction handles downstream normalization.
    \item \textbf{SVLM (Qwen2-VL)}: Natively multilingual; handles Vietnamese
          diacritics through the base model's multilingual pre-training without
          additional OCR configuration.
\end{itemize}

Vietnamese diacritical challenges (character confusion between ``O'' vs.\ ``\^{O}'',
``a'' vs.\ ``\u{a}'') are mitigated by the SLM reasoning stage, where
Vietnamese-specific correction patterns supplement the existing English correction table.

\subsubsection{VLM Extraction and Vietnamese Chart Labels}

With the VLM-based Stage~3, extraction no longer relies on OCR or keyword-based role
classification. The DePlot and MatCha backends were trained primarily on English chart
data; performance may decrease for charts with Vietnamese axis labels or legends. Two
mitigation strategies are available:

\begin{itemize}
    \item \textbf{SVLM backend}: The Qwen2-VL extractor supports multilingual output
          through system prompt configuration. Setting the system prompt to request
          Vietnamese table output enables direct Vietnamese extraction.
    \item \textbf{Stage~4 correction}: The SLM reasoning stage receives both the VLM
          table output and chart type metadata, and can apply Vietnamese-specific
          correction patterns independently of the extraction backend.
\end{itemize}

The configuration change for Vietnamese extraction is limited to
\texttt{pipeline.yaml: extraction.extractor\_backend: "svlm"} and updating the
Stage~4 system prompt language parameter, with no core pipeline code modification.
