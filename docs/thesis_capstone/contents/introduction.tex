...
\begin{enumerate}
    ...
    \item \textbf{VLM Extraction for structured data}: Vision-Language Models
          (DePlot \cite{liu2023deplot}, MatCha \cite{lee2023matcha}, Pix2Struct
          \cite{lee2023pix2struct}) trained on chart-to-table derendering convert
          chart pixels directly into structured tables, replacing the initial
          geometry-based pipeline (which failed on 0/40 non-pie axis detections
          in a 50-chart benchmark).
...
\begin{itemize}
    ...
    \item Build a large-scale training dataset ($> 200{,}000$ samples) from academic papers
          for SLM fine-tuning.
    \item Demonstrate that the hybrid neural-VLM approach using chart-trained
          VLM extraction outperforms both pure geometry-based and pure end-to-end
          estimation methods in extraction reliability, while running locally on
          consumer hardware (NVIDIA RTX 3060, 6~GB VRAM).
...
\begin{itemize}
    ...
    \item VLM extraction latency is 2--4~s per chart on GPU; charts requiring the
          SVLM (Qwen2-VL) multilingual backend incur higher latency due to larger
          model size ($\sim$2B parameters).
