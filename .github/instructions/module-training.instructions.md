...
**Key Files:**
- `scripts/prepare_slm_training_data.py` - Data pipeline (QA pairs + Stage 3 features -> training format)
- `scripts/train_slm_lora.py` - LoRA fine-tuning with PEFT
- `scripts/test_qwen_slm.py` - Inference testing
- `scripts/evaluate_slm.py` - Benchmark evaluation (NEW)
- `config/models.yaml` - Model paths, hyperparameters
- `config/training.yaml` - Training configuration (NEW)
- `data/slm_training/` - Processed training datasets
...
### 3.1. Data Sources (Already Available)

| Source | Records | Format | Location |
| --- | --- | --- | --- |
| Chart QA pairs (v2) | 32,445 | JSON (question-answer) | `data/academic_dataset/chart_qa_v2/` |
| Stage 3 features | 32,445 | JSON (OCR + geometry) | `data/academic_dataset/stage3_features/` |
| SLM training set | 84 | ChatML conversations | `data/slm_training/` |
...
**Current status:** Script EXISTS but only produces 84 conversations (bar charts only). Needs expansion to all 8 chart types.
...
```bash
# Step 1: Prepare training data (expand beyond bar charts)
...
.venv/Scripts/python.exe scripts/train_slm_lora.py \
    --data-dir data/slm_training \
    --output-dir models/slm/qwen2.5-1.5b-chart-lora \
    --epochs 3 \
    --batch-size 4 \
    --lora-rank 16
...
.venv/Scripts/python.exe scripts/evaluate_slm.py \
    --model-path models/slm/qwen2.5-1.5b-chart-lora/final \
    --test-data data/slm_training/test.json \
    --output models/evaluation/slm_eval_results.json

# Step 5: Merge LoRA adapters into base model (for deployment)
.venv/Scripts/python.exe scripts/merge_slm_lora.py \
    --base-model Qwen/Qwen2.5-1.5B-Instruct \
    --lora-path models/slm/qwen2.5-1.5b-chart-lora/final \
    --output-dir models/slm/qwen2.5-1.5b-chart-merged
```
